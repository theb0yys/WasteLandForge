...
- `gates/gate-488-live-mo2-compatibility-smoke-deferred.md` - Gate 488, completed a read-only live-MO2 preflight, found only Fallout 4 London instances and no authorized FNV test instance, deferred all external mutation, and routed to the next repository-local value slice.
- `gates/gate-489-deterministic-bsa-packing-plan-contract.md` - Gate 489, defined a fresh-package-backed deterministic BSA classification and tool-neutral recipe contract with plugin-associated names, uncompressed archives, loose/refused/KF handling, provenance, diagnostics, and no packer execution.
- `gates/gate-490-deterministic-bsa-packing-plan-implementation.md` - Gate 490, implemented the deterministic BSA plan CLI and desktop lane with immutable schema, exact package/plugin provenance, declarative flags, 851 passing tests, publication, and installed no-packer regression.
- `gates/gate-491-bsa-plan-verification-and-candidate-integration.md` - Gate 491, added read-only BSA-plan verification, seven-stage Candidate integration, release-preparation digest binding, desktop evidence actions, and 853 passing tests without changing the FOMOD payload or invoking a packer.
- `gates/gate-492-bsarch-adapter-contract.md` - Gate 492, selected the upstream TES5Edit BSArch CLI and defined explicit provider identity, preview approval, isolated inputs, uncompressed FNV invocation, repeat-pack determinism, list/unpack verification, provenance, and no-install boundaries.
- `gates/gate-493-bsarch-provider-preview-implementation.md` - Gate 493, implemented explicit BSArch provider inspection, immutable approval-bound CLI/desktop dry-run preview, synthetic probe abstraction, schema coverage, and no-process/no-write safeguards.
- `gates/gate-494-approval-bound-bsarch-execution-orchestration.md` - Gate 494, implemented approval-bound injectable pack/list/unpack execution with isolated inputs, repeatability and byte verification, atomic promotion, invocation provenance, schemas, and synthetic failure preservation.
- `gates/gate-495-bounded-bsarch-runner-and-approval-ux.md` - Gate 495, implemented the bounded Windows runner, exact CLI approval, stale-safe desktop Build BSA workflow, controlled synthetic provider fixture, 859 passing tests, publication, installer rebuild, and installed execution regression.
- `gates/gate-496-real-bsarch-compatibility-smoke-deferred.md` - Gate 496, completed read-only real-provider discovery, found no user-supplied upstream bsarch.exe, made no external changes or compatibility claims, and deferred the smoke behind an exact path plus explicit authorization.
- `gates/gate-497-optional-bsa-execution-lane-closeout.md` - Gate 497, closed the optional BSA execution lane at its synthetic-provider boundary, confirmed bsa-build cannot enter mandatory Candidate/release payloads, and selected an opt-in verified BSA-backed package variant as the next value slice.
- `gates/gate-498-opt-in-bsa-backed-package-contract.md` - Gate 498, defined the opt-in bsa-package command, exact current package/plan/build evidence binding, complete packed-loose partition, deterministic local package layout, immutable provenance, diagnostics, and strict exclusion from mandatory release policy.
- `gates/gate-499-opt-in-bsa-backed-package-implementation.md` - Gate 499, implemented the opt-in bsa-package assembler, exact evidence/partition verification, deterministic ZIP and immutable provenance, CLI/desktop workflow, 861 passing tests, publication, installer rebuild, and installed archive-bearing synthetic proof outside release policy.
- `gates/gate-500-existing-bsa-package-verification-closeout.md` - Gate 500, added project-independent read-only bsa-package verification, immutable manifest/checksum/staging/ZIP byte checks, tamper and no-write proof, installed regression, lane closeout, and next-value routing to xEdit report ingestion.
- `gates/gate-501-xedit-check-report-ingestion-contract.md` - Gate 501, selected xEdit Check(e) record-error reports as the first authoritative family and defined the Forge-owned JSON envelope, stable file-local identity, strict script/plugin provenance, typed diagnostics, safety limits, fixtures, and no-launch/no-mutation boundary.
- `gates/gate-502-xedit-check-report-implementation.md` - Gate 502, implemented deterministic Check(e) script generation, digest-bound report ingestion, WF-GEN-015/WF-SEM-045 diagnostics, desktop/Candidate evidence, 864 passing tests, publication, and installed no-xEdit/no-mutation proof.
- `gates/gate-503-real-xedit-check-compatibility-smoke-deferred.md` - Gate 503, completed bounded read-only xEdit discovery, found no configured or supplied real FNV-capable provider, deferred the manual compatibility smoke, and made no external or plugin changes.
- `gates/gate-504-xedit-check-lane-closeout-and-next-value-routing.md` - Gate 504, closed xEdit Check ingestion at its deterministic synthetic boundary and selected a guided Basic Mod Builder as the next major user-value slice.
- `gates/gate-505-guided-basic-mod-builder-contract.md` - Gate 505, defined a preview-bound transactional Basic Mod Builder for MCM-only or MCM+JIP source creation, validation, combined packaging, verified FOMOD output, cleanup, and installed proof.
- `gates/gate-506-guided-basic-mod-builder-implementation.md` - Gate 506, implemented and installed-tested the transactional Basic Mod Builder for MCM-only or MCM+JIP source, validation, combined packaging, and verified FOMOD output.
- `gates/gate-507-basic-mod-builder-reopen-closeout-and-next-value-routing.md` - Gate 507, proved Basic Mod Builder projects reopen for specialist MCM/JIP edits and deterministic FOMOD rebuild, closed the lane, and selected a guided plugin-backed mod workbench next.
- `gates/gate-508-guided-plugin-backed-mod-workbench-contract.md` - Gate 508, defined a seven-phase evidence-derived plugin mod workbench and preview-gated revised-plugin intake that resets review without editing plugin records.
- `gates/gate-509-plugin-mod-workbench-and-revised-plugin-implementation.md` - Gate 509, implemented the seven-phase Plugin Mod Workbench, atomic revised-plugin intake and review reset, iterative digest-specific review evidence, 872 passing tests, publication, and installed Candidate invalidation proof.
- `gates/gate-510-plugin-mod-workbench-hardening-and-closeout.md` - Gate 510, added independent package/FOMOD freshness, direct cancellable build/Candidate actions, installed proof, and closed the plugin workbench lane.
- `gates/gate-511-installed-product-usability-and-release-readiness-audit.md` - Gate 511, audited the installed guided workflows, identified 17-tab navigation and legacy startup/product copy as the concrete release-usability blocker, and routed desktop information-architecture consolidation.
- `gates/gate-512-grouped-desktop-navigation-implementation.md` - Gate 512, implemented persistent Build/Review/System navigation, Basic Mod Builder startup, accurate product copy, publication, installer rebuild, and installed navigation regression.
- `gates/gate-513-responsive-navigation-accessibility-closeout.md` - Gate 513, fixed minimum-height sidebar clipping, verified grouped navigation at 960x640 and 1180x760 in the installed app, and closed desktop usability work.
- `gates/gate-514-v0-1-scope-freeze-and-user-test-handoff.md` - Gate 514, froze v0.1 scope, accounted for explicit MO2/BSArch/xEdit deferrals, and published the exact local user-test handoff and known limitations.
- `gates/gate-515-user-test-defect-intake-awaiting-evidence.md` - Gate 515, fixed forge init recovery through pre-created empty scaffold directories while retaining strict refusal of files and unplanned contents, then rebuilt and installed-tested the candidate.
- `gates/gate-516-plugin-only-geck-handoff-fix.md` - Gate 516, fixed WF-GEN-011 for validated plugin-only projects with digest-bound opaque plugin provenance and a GECK authoring worklist; publication was completed by Gate 517.
- `gates/gate-517-plugin-only-geck-handoff-publication-closeout.md` - Gate 517, passed 875 tests, rebuilt the unsigned installer, and proved the installed plugin-only handoff and preview-gated GECK launch without changing plugin bytes.
- `gates/gate-518-plugin-only-user-test-handoff-refresh.md` - Gate 518, corrected the v0.1 user-test installer hash and documented the verified plugin-only GECK handoff workflow without reopening feature scope.
- `gates/gate-519-greenfield-geck-handoff.md` - Gate 519, added schema-versioned greenfield GECK handoffs and proved installed explicit launch before any ESP/ESM exists, with 878 passing tests and no plugin generation.
- `gates/gate-520-geck-authoring-plan-authority-and-contract.md` - Gate 520, adopted ADR-013 and defined a preview-only, evidence-resolved GECK authoring plan plus verifier-before-writer gate sequence without authorizing plugin mutation.
- `gates/gate-521-preview-only-geck-authoring-plan.md` - Gate 521, implemented immutable intent/plan schemas and deterministic CLI planning with digest-bound local evidence, dry-run no-write proof, WF-GEN-016 refusal, and 882 passing tests without external execution.
- `gates/gate-522-read-only-geck-authoring-semantic-verification.md` - Gate 522, implemented the immutable xEdit semantic verification report contract and deterministic read-only parser with exact plan/provider/script/plugin provenance and first-slice CONT/REFR postcondition checks, without launching xEdit or writing plugin bytes.
- `gates/gate-523-geck-editor-provider-discovery.md` - Gate 523, identified the local GECK 1.4.0.518/xNVSE 6.4.4/xEdit 4.1.5 provider set and public xNVSE editor-plugin host, found no verifiable GECK Extender installation or supported high-level authoring API, and blocked Gate 524 real execution pending provider-feasibility remediation.
- `gates/gate-524-authoring-provider-sufficiency-and-remediation.md` - Gate 524, applied the Gate 520 sufficiency condition, refused authoring execution, pinned the installed xNVSE release evidence, identified the GECK Extender source/binary and v120 toolset gaps, and defined the separately approved no-mutation provider probe contract.
- `gates/gate-525-external-provider-acquisition-and-compatibility-preflight.md` - Gate 525, correlated the official xNVSE 6.4.4 release with all installed binaries, preflighted the unmodified Win32 GECK example through shared-library compilation before its missing-v143 failure, confirmed the GECK Extender 0.52/available-source 0.31 mismatch, and kept provider execution blocked.
- `gates/gate-526-provider-baseline-decision.md` - Gate 526, selected a pinned xNVSE-only Win32 host-loading probe through an explicitly selected MO2 VFS profile, retained GECK Extender as a preferred future authoring enhancement, required upstream v143 without retargeting, and kept build, staging, launch, and record mutation in separate gates.
- `gates/gate-527-xnvse-host-probe-build.md` - Gate 527, installed and verified the exact v143 14.44 toolchain, built the Forge-owned xNVSE-only Win32 GECK host probe against pinned external source, proved its architecture/exports/dependencies/repeatability, and emitted an ignored local manifest without staging or execution.
- `gates/gate-528-geck-host-probe-live-smoke-contract.md` - Gate 528, defined separate plan/staging/environment/launch approvals, probe-specific MO2 request and effective-provider checks, protected pre/post manifests, and fail-closed recovery; read-only preflight found no initialized FNV MO2 profile and left staging/execution blocked.
- `gates/gate-529-geck-host-probe-no-launch-planning.md` - Gate 529, implemented the immutable private run-plan parser, launch-authorized build and create-only staging previews, deterministic protected manifests, and synthetic read-only effective-provider preflight with hard no-write/no-launch safety; the real environment remains blocked.
- `gates/gate-530-mo2-virtual-file-tree-adapter.md` - Gate 530, connected the effective-provider evaluator to the documented read-only MO2 virtual-tree and winner-resolution APIs, verified API-faithful synthetic refusal paths, and refreshed the deterministic companion package without live MO2 execution.
- `gates/gate-531-live-mo2-adapter-compatibility-preflight.md` - Gate 531, deferred the live read-only adapter check after confirming no initialized FNV MO2 instance/profile or installed companion exists; no MO2 state or external process changed.
- `gates/gate-532-first-slice-xedit-semantic-verifier-producer-contract.md` - Gate 532, selected a two-stage read-only xEdit observation producer and Forge report sealer for the existing Gate 522 authoring-verification contract, avoiding script self-hash while keeping all real xEdit execution separately gated.
- `gates/gate-533-deterministic-xedit-verifier-producer.md` - Gate 533, implemented the deterministic read-only Pascal observer bundle, immutable raw-observation schema, Forge report sealer, CLI/help/explain surface, and synthetic acceptance/refusal coverage with 901 passing tests and no xEdit or plugin execution.
- `gates/gate-534-fnvedit-verifier-compatibility-smoke-deferred.md` - Gate 534, deferred the manual FNVEdit verifier smoke because no explicitly supplied redistributable synthetic plugin, exact provider path, project/bundle selection, raw-evidence destination, or digest-bound launch approval was available; no external process or plugin operation occurred.
- `gates/gate-535-geck-authoring-verifier-lane-closeout-and-desktop-routing.md` - Gate 535, closed the GECK authoring verifier lane at its deterministic synthetic boundary and selected a post-v0.1 desktop review workflow contract as the next repository-local value slice without reopening external execution.
- `gates/gate-536-desktop-geck-authoring-review-workflow-contract.md` - Gate 536, defined an implementation-ready nested GECK authoring plan/verification desktop workflow using the existing Review route, canonical backend commands, digest-bound previews, synthetic tests, preserved manual handoff ownership, and no external execution.
- `gates/gate-537-desktop-geck-authoring-review-workflow-implementation.md` - Gate 537, implemented the bounded nested GECK authoring plan/verification desktop workflow, strict canonical backend bridge, stale-safe digest previews, read-only Project Outputs evidence, and synthetic coverage with 912 passing tests and no external execution.
- `gates/gate-538-installed-geck-authoring-review-regression.md` - Gate 538, published and installed the self-contained app, bundled a compatibility-neutral synthetic authoring demo, proved the complete installed review workflow and preserved manual handoff regression, passed 912 tests, and changed no opaque plugin bytes.
- `gates/gate-539-desktop-geck-authoring-review-lane-closeout.md` - Gate 539, closed the installed synthetic GECK authoring review lane, retained Gate 531/Gate 534 deferrals, and selected a guided first-slice intent and local-evidence builder as the next repository-local value slice.
- `gates/gate-540-guided-geck-authoring-intent-and-local-evidence-builder-contract.md` - Gate 540, defined the implementation-ready existing-project GECK Intent Builder contract with lossless manifest migration, bounded text-evidence attachment, stale-safe source transactions, recovery/undo, canonical backend checks, and routing into the existing review workflow.
- `gates/gate-541-guided-geck-intent-builder-implementation.md` - Gate 541, implemented, published, and installed-proved the guided GECK Intent Builder with lossless migration, bounded evidence, transactional recovery/undo, provisional/resolved states, and Authoring Review routing without editor execution or plugin writes.
- `gates/gate-542-guided-geck-intent-builder-lane-closeout-and-user-test-handoff.md` - Gate 542, closed the installed synthetic GECK Intent Builder lane, refreshed the exact local user-test handoff, retained Gate 531/Gate 534/provider deferrals, and selected observation-driven defect intake without authorizing external execution or plugin mutation.
- `gates/gate-543-local-fnv-game-knowledge-catalogue-contract.md` - Gate 543, defined an implementation-ready private local `FalloutNV.esm` record catalogue with xEdit export evidence, offline search, cell/worldspace/reference context, provenance receipts, and provisional GECK Intent Builder handoff without redistributing game data or launching external tools.
- `gates/gate-544-local-fnv-game-knowledge-catalogue-implementation.md` - Gate 544, implemented, published, and installed-proved the private FNV game-knowledge catalogue, bounded read-only xEdit producer, strict local index/search/receipts, and provisional intent handoff with 945 passing tests and no real editor or game execution.
- `gates/gate-545-local-fnvedit-game-knowledge-compatibility-smoke-approval-pending.md` - Gate 545, ran the separately approved exact FNVEdit smoke, failed closed when the active plugin list loaded and no export was emitted, proved game Data/core evidence unchanged, and recorded provider log/settings side effects.
- `gates/gate-546-private-single-master-fnvedit-execution-contract.md` - Gate 546, defined the no-launch remediation: an immutable automated evidence lineage, private one-master plugins list and provider state, documented xEdit arguments, digest approval, visible process states, strict side-effect audit, and exact Gate 545 cleanup policy.
- `gates/gate-547-private-single-master-fnvedit-execution-implementation.md` - Gate 547, implemented the private one-master execution planner/runner, immutable automated evidence lineage, visible two-click digest approval, strict side-effect audit, 962 passing tests, publication, installer rebuild, and installed controlled-provider proof without running real FNVEdit.
- `gates/gate-548-private-single-master-fnvedit-compatibility-preview-approval-pending.md` - Gate 548 executed the exact separately approved plan once with no retry, observed FNVView fail with no INI, preserved the provider-created log, and failed closed because protected provider state changed and no private export/log was emitted.
- `gates/gate-549-fnvedit-ini-routing-failure-closeout.md` - Gate 549 traced the Gate 548 failure to the omitted documented `-I` game-INI input, preserved cache/log behavior as unresolved fail-closed evidence, and defined a versioned synthetic Gate 550 remediation without launching or cleaning up FNVEdit.
- `gates/gate-550-ini-bound-private-execution-contract.md` - Gate 550 implemented immutable `0.2.0` INI-bound plan/receipt contracts, exact `-I` routing and protected INI audit, desktop configuration and preview, 969 passing tests, publication, and installed controlled-provider proof without running real FNVEdit.
- `gates/gate-551-real-fnvedit-ini-bound-preview-configuration-pending.md` - Gate 551 completed a no-write preflight, found the configured master/provider/user-state and bounded `Fallout.ini` suggestion present but the explicit saved INI setting empty, and stopped before hashing, preview creation, or provider launch.
- `gates/gate-552-geck-provider-api-discovery-and-compatibility-proof.md` - Gate 552 revalidated the pinned xNVSE GECK host and current upstream API evidence, proved that no supported editor authoring surface covers the first-slice `CONT`/`REFR` operations, and kept writer execution blocked without invoking reverse-engineered internals.
- `gates/gate-553-fnvedit-verifier-compatibility-proof-approval-pending.md` - Gate 553 preflighted the independent verifier smoke, confirmed the exact FNVEdit candidate but found no approved redistributable synthetic plugin, matching observer bundle, contained observation destination, saved INI path, or digest-bound run approval, and stopped at Approval A pending.
- `gates/gate-554-redistributable-synthetic-fnv-fixture-acquisition.md` - Gate 554 audited the official MPL-licensed xEdit hardcoded FNV plugin, rejected it because it has no first-slice `CONT`/`REFR` semantics, and defined the human-GECK-authored licensed fixture intake required to resume Gate 553 without making Forge a plugin writer.
...
- Gate 558 - implement explicit placement-transform evidence binding, immutable `0.2.0` intent/plan/subject contracts, strict cross-checking, Intent Builder migration and UX, tests, publication, and installed no-execution proof.
- Gate 531 resume - requires an operator-created FNV MO2 instance, explicitly named already-current probe profile, installed Gate 530 companion, and separate read-only approval. No Gate 532 authoring execution is authorized while these prerequisites remain unresolved.
- Gate 534 resume - requires an exact approved FNVEdit path, explicitly supplied redistributable synthetic plugin, matching Gate 533 project/observer bundle, project-contained raw-evidence destination, and digest-bound launch approval. No authoring execution follows automatically.
- Gate 551 resume - after saving the explicit `Fallout.ini` setting and separately authorizing preview preparation, create one fresh exact real-provider preview and stop at its displayed digest without execution.
- Gate 553 Approval A - after Gate 554's human-GECK-authored three-file fixture package exists, supply its exact path plus the matching Forge project, contained raw-observation destination, existing `Fallout.ini`, and selected FNVEdit provider. Approval A prepares a digest-bound preview only; a separate Approval B is required for one no-retry run.
...
- Gate 495 - implement a bounded concrete Windows process runner and explicit approval UX, prove it with a controlled synthetic executable in installed regression, and keep any real BSArch smoke separately authorized.
- Gate 496 - perform a separately authorized compatibility smoke with a user-supplied upstream bsarch.exe against an isolated synthetic project, or record the gate deferred if no authorized provider is available.
- Gate 497 - close the optional BSA execution lane for v0.1 with real-provider compatibility explicitly deferred, keep unverified BSA output out of mandatory release inputs, and select the next repository-local value slice.
- Gate 498 - research and define an opt-in BSA-backed package variant from currently verified bsa-build archives, the associated reviewed plugin, and deliberate loose files, without changing mandatory Candidate or release policy.
- Gate 499 - implement the opt-in bsa-package vertical slice, immutable evidence schema, exact partition verifier, deterministic staging/ZIP, CLI and desktop workflow, synthetic regressions, publication, and installed proof while keeping it outside mandatory release policy.
- Gate 500 - add project-independent read-only verification for an existing bsa-package folder and close the optional package lane without rebuilding it or changing mandatory release policy.
- Gate 501 - research and define the first read-only xEdit report-ingestion contract for typed diagnostics and provenance, without launch, patching, conflict resolution, plugin mutation, load-order changes, or proprietary fixtures.
- Gate 502 - implement the synthetic end-to-end xEdit Check-report slice, immutable schemas, deterministic read-only Pascal script, strict provenance parser, WF-GEN-015/WF-SEM-045 diagnostics, handoff evidence, CLI/desktop review, publication, and installed no-xEdit/no-mutation regression.
- Gate 503 - perform a read-only local xEdit/FNV compatibility preflight and either run a separately authorized manual smoke or record it deferred and close the xEdit Check lane.
- Gate 504 - close the xEdit Check ingestion lane at its deterministic synthetic boundary and route to the next repository-local mod-building value slice.
- Gate 505 - research and define a guided Basic Mod Builder that creates, authors, validates, packages, and exposes a minimal MCM/JIP mod through one desktop workflow.
- Gate 506 - implement the guided Basic Mod Builder, transactional source/package pipeline, desktop workspace, tests, publication, installer, and installed FOMOD regression.
- Gate 507 - verify project reopen, specialist edit, deterministic rebuild handoff, close the Basic Mod Builder lane, and select the next major authoring value slice.
- Gate 508 - research and define a guided plugin-backed mod workbench spanning narrative source, GECK handoff, opaque plugin intake, xEdit review approval, FOMOD, and Candidate readiness without plugin mutation.
- Gate 509 - implement the Plugin Mod Workbench phase projection, specialist routing, revised-plugin intake/reset, tests, publication, installer, and installed synthetic progression proof.
- Gate 510 - harden workbench package freshness, direct safe actions, primary-plugin UI preference, installed restart proof, and close the plugin workbench lane.
- Gate 511 - audit installed end-to-end usability and release readiness across both guided builders, fix only observed blockers, and route the next major value slice.
- Gate 512 - consolidate desktop navigation into Build/Review/System groups, route configured startup to Basic Mod Builder, update global product copy, and prove responsive installed navigation without removing specialist workspaces.
- Gate 513 - verify grouped navigation visually and accessibly at minimum/standard sizes, fix observed defects, and close the desktop usability lane.
- Gate 514 - freeze v0.1 scope, account for open/deferred work, and produce the exact user-test build and known-limitations handoff without adding features.
- Gate 515 - observation-driven defect triage only after a reproducible user-test report or explicit authorization for one deferred external compatibility environment.
