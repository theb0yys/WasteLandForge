...
internal static class CliHelpWriter
{
    ...
    private static void WritePackageHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Usage:");
        writer.WriteLine("  forge package [project-root] [--project <path>] [--target mcm-json] [--output dist/<name>] [--format human|plain|json] [--dry-run] [--no-input]");
        writer.WriteLine();
        writer.WriteLine("Assembles the Gate 85 deterministic MCM Extender package staging tree under project dist/. Target 'mcm-json' writes MCM JSON, translations, staged referenced texture assets, schema-validated package-manifest.json, schema-validated install-preview.json, install-preview.md, schema-validated package-verification.json, package-verification.md, reusable package-verification evidence cross-checks, entry-validated package.zip, build-manifest.json, and checksums.sha256.");
        ...
        writer.WriteLine();
        writer.WriteLine("Exit codes:");
        writer.WriteLine("  0 package written or planned");
        writer.WriteLine("  1 blocking diagnostics found");
        writer.WriteLine("  2 usage or unsupported format");
    }
