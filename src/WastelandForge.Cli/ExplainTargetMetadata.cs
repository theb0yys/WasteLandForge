    ...
    private static readonly IReadOnlyDictionary<string, ExplainTargetMetadata> Targets =
        new Dictionary<string, ExplainTargetMetadata>(StringComparer.Ordinal)
        {
            ...
            ["geck-authoring-plan"] = Target(
                ...
                "authoring-plan",
                "implemented",
                "Preview-only deterministic GECK authoring plan with exact operator-attested placement lineage in version 0.2.0.",
                "docs/adr/ADR-013.md",
                ...
                ["WF-GEN-016"],
                ["Does not execute GECK or xEdit.", "Does not write plugin bytes or game Data.", "Refuses provisional, stale, or placement-mismatched evidence."]),
            ["geck-authoring-subject-handoff"] = Target(
                "geck-authoring-subject-handoff",
                "GECK verifier subject handoff",
                "manual-authoring-handoff",
                "implemented",
                "Deterministic manual GECK authoring kit requiring plan 0.2.0 and its exact placement-evidence lineage.",
                "WasteLandForge/planning/gates/gate-555-synthetic-verifier-subject-plan-and-manual-geck-authoring-handoff.md",
                ["forge generate --target geck-authoring-subject-handoff"],
                ["generated/geck-authoring-plan/subject-handoff"],
                ["subject-contract.json", "worklist.md", "creation-notes.template.md", "build-manifest.json", "checksums.sha256"],
                ["tool.geck", "tool.xedit.record_inspection"],
                ["WF-GEN-019"],
                ["Does not execute GECK or xEdit.", "Does not create, parse, verify, approve, or import plugin bytes.", "Does not select or grant a redistribution license."]),
            ["geck-authoring-verifier"] = Target(
                "geck-authoring-verifier",
                "GECK authoring xEdit observer",
                "verification-observer",
                "implemented",
                "Deterministic read-only Pascal observer bundle for the first-slice GECK authoring plan.",
                "WasteLandForge/planning/gates/gate-533-deterministic-xedit-verifier-producer.md",
                ["forge generate --target geck-authoring-verifier"],
                ["generated/geck-authoring-plan/verification"],
                ["verifier.pas", "observer-contract.json", "observer-manifest.json", "checksums.sha256"],
                ["tool.xedit.record_inspection"],
                ["WF-GEN-017"],
                ["Does not execute xEdit.", "Does not write plugin bytes or game Data.", "Produces no semantic verification report without supplied observations."]),
            ["geck-authoring-verification"] = Target(
                "geck-authoring-verification",
                "GECK authoring verification sealer",
                "verification-report",
                "implemented",
                "Validates immutable raw observations, seals the Gate 522 report contract, and applies exact first-slice semantic postconditions.",
                "WasteLandForge/planning/gates/gate-533-deterministic-xedit-verifier-producer.md",
                ["forge generate --target geck-authoring-verification --observations <project-relative-path>"],
                ["generated/geck-authoring-plan/verification"],
                ["report.json"],
                ["tool.xedit.record_inspection"],
                ["WF-GEN-017", "WF-SEM-046"],
                ["Does not execute xEdit.", "Does not write plugin bytes or game Data.", "Refuses stale, incomplete, unsafe, or unsupported observations."]),
            ["docs"] = Target(
                "docs",
