...
- Gate 337 - plan local standalone Forge executable distribution so generated consumer projects have a source-agnostic way to satisfy the `forge` on `PATH` prerequisite, still stopping before executable packaging changes, installer creation, NuGet publication, package restore, external tool execution, runtime probes, signing or attestation, and AI behavior.
