...
internal static class CliHelpWriter
{
    ...
    private static void WriteGenerateHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Usage:");
        writer.WriteLine("  forge generate [project-root] [--project <path>] [--target reports|mcm-json|jip-scripts|xedit-audit|xedit-audit-report-handoff|geck-authoring-plan|geck-authoring-subject-handoff|geck-authoring-verifier|geck-authoring-verification] [--observations <project-relative-path>] [--output generated/<name>] [--format human|plain|json] [--dry-run] [--no-input]");
        writer.WriteLine();
        ...
        writer.WriteLine("Target 'geck-authoring-plan' writes only deterministic plan evidence under generated/geck-authoring-plan. Intent 0.2.0 requires digest-bound operator-attested placement evidence matching the GECK provider, cell identity, and exact transform. --dry-run performs no writes. It never launches GECK/xEdit or writes plugin bytes.");
        writer.WriteLine("Target 'geck-authoring-subject-handoff' requires plan 0.2.0 and writes a deterministic manual GECK worklist, subject contract, creation-notes template, manifest, and checksums bound to the current plan and placement evidence. Legacy plan 0.1.0 remains readable but cannot produce an operator-ready subject. It never launches tools, writes plugin bytes, verifies a plugin, grants approval, or selects a license.");
        writer.WriteLine("Target 'geck-authoring-verifier' writes a deterministic read-only Pascal observer, contract, manifest, and checksums. Target 'geck-authoring-verification' requires --observations, seals the existing report contract, and runs semantic verification. Both support no-write --dry-run and never launch xEdit or write plugin bytes.");
        writer.WriteLine();
        ...
        writer.WriteLine("  generated/geck-authoring-plan/verification/verifier.pas");
        writer.WriteLine("  generated/geck-authoring-plan/verification/observer-contract.json");
        writer.WriteLine("  generated/geck-authoring-plan/verification/observer-manifest.json");
        writer.WriteLine("  generated/geck-authoring-plan/verification/checksums.sha256");
        writer.WriteLine("  generated/geck-authoring-plan/verification/report.json");
        writer.WriteLine("  generated/geck-authoring-plan/subject-handoff/subject-contract.json");
        writer.WriteLine("  generated/geck-authoring-plan/subject-handoff/worklist.md");
        writer.WriteLine("  generated/geck-authoring-plan/subject-handoff/creation-notes.template.md");
        writer.WriteLine();
        writer.WriteLine("Examples:");
        ...
        writer.WriteLine("  forge generate fixtures/projects/GeckAuthoringPlanExample --target geck-authoring-verifier --format json");
        writer.WriteLine("  forge generate fixtures/projects/GeckAuthoringPlanExample --target geck-authoring-subject-handoff --dry-run --format json");
        writer.WriteLine("  forge generate fixtures/projects/GeckAuthoringPlanExample --target geck-authoring-verification --observations evidence/observations.json --dry-run --format json");
        writer.WriteLine();
        writer.WriteLine("Exit codes:");
    ...
    private static void WriteBuildHelp(TextWriter writer)
    {
        ...
        writer.WriteLine("  dist/build/build-plan.json");
        writer.WriteLine("  dist/build/build-plan.md");
        writer.WriteLine("  dist/build/build-report.json");
        writer.WriteLine("  dist/build/build-report-index.json");
        writer.WriteLine("  dist/build/build-report-index.md");
    ...
    private static void WritePackageHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Usage:");
        writer.WriteLine("  forge package [project-root] [--project <path>] [--target reports|mcm-json|jip-scripts|mod-package|geck-handoff|fomod|bsa-plan|bsa-bsarch|bsa-package] [--bsa-plugin <Data-relative ESP/ESM>] [--packer <absolute-bsarch.exe>] [--output dist/<name>] [--mo2-mods-root <path> --mo2-mod-name <name>] [--reuse-existing-package] [--summary <path>] [--format human|plain|json|sarif|github] [--dry-run] [--verify-existing] [--no-input]");
        writer.WriteLine();
        writer.WriteLine("Assembles deterministic package staging trees under project dist/. Existing targets remain unchanged. Target 'mod-package' rebuilds declared MCM and JIP sources, rejects normalized Data-path collisions, and writes one schema-validated Data-relative ZIP plus install, build-manifest, and checksum evidence under dist/mod-package without installing it.");
        writer.WriteLine("For target 'mod-package', paired --mo2-mods-root and --mo2-mod-name options export validated loose files into one new named MO2 mod directory. Existing destinations, Overwrite paths, unsafe names, and reparse roots are refused. Forge does not enable the mod or mutate profiles, priority, load order, plugins, game Data, or MO2 Overwrite. Use --dry-run for a no-export preview.");
        writer.WriteLine("Target 'bsa-plan' rebuilds the validated combined package, requires exactly one reviewed ESP/ESM (or --bsa-plugin selection), classifies packable and loose-only entries, and writes a deterministic tool-neutral recipe under dist/bsa-plan. It creates no BSA and executes no packer.");
        writer.WriteLine("Use --target bsa-plan --verify-existing to revalidate the immutable plan schema, checksums, source package and plugin digests, exact staged bytes, and no-BSA/no-packer boundary without regenerating outputs.");
        writer.WriteLine("Target 'bsa-bsarch' uses --packer and --dry-run to return an approval SHA without process execution. Run again without --dry-run and with --approve <sha256> to execute that exact current preview through the bounded no-shell BSArch runner.");
        writer.WriteLine("Target 'bsa-package' assembles a local deterministic ZIP from a currently verified bsa-build plus the plan's deliberate loose files. Provider compatibility remains unverified and this output is not a Release Candidate input.");
        writer.WriteLine("Use --target bsa-package --verify-existing with the existing dist/bsa-package folder to verify manifest, checksums, staging and ZIP bytes read-only without project source or rebuilding.");
        writer.WriteLine("Use --reuse-existing-package only with a named mod-package export to cryptographically revalidate and reuse dist/mod-package without rewriting it. Candidate callers can bind expected package/build manifest SHA-256 and lengths. This is distinct from MCM-only --verify-existing.");
        writer.WriteLine("Target 'geck-handoff' writes deterministic greenfield, plugin-only, or narrative GECK worklists plus provenance. It does not create plugin records, compile scripts, mutate plugins, or launch GECK/xEdit.");
        writer.WriteLine("Target 'fomod' rebuilds the validated combined payload and writes the unconditional required-files FOMOD 5.0 profile. It emits no installer choices, conditions, scripts, manager commands, game Data writes, or MO2/Vortex launch behavior.");
        ...
        writer.WriteLine();
        writer.WriteLine("Outputs:");
        writer.WriteLine("  dist/reports-package/package-plan.json");
        writer.WriteLine("  dist/reports-package/staging/package-layout.json");
        writer.WriteLine("  dist/reports-package/package.zip");
        writer.WriteLine("  dist/reports-package/package-archive-evidence.json");
        writer.WriteLine("  dist/reports-package/build-manifest.json");
        writer.WriteLine("  dist/reports-package/checksums.sha256");
        writer.WriteLine("  dist/mcm-json/MCM/<menu>.json");
        writer.WriteLine("  dist/mcm-json/MCM/Translations/<modName>.ini");
        writer.WriteLine("  dist/mcm-json/<asset-target>");
        writer.WriteLine("  dist/mcm-json/package-manifest.json");
        ...
        writer.WriteLine("  dist/jip-scripts/build-manifest.json");
        writer.WriteLine("  dist/jip-scripts/checksums.sha256");
        writer.WriteLine("  dist/fomod/staging/fomod/info.xml");
        writer.WriteLine("  dist/fomod/staging/fomod/ModuleConfig.xml");
        writer.WriteLine("  dist/fomod/package.zip");
        writer.WriteLine("  dist/fomod/fomod-manifest.json");
        writer.WriteLine("  dist/fomod/build-manifest.json");
        writer.WriteLine("  dist/fomod/checksums.sha256");
        writer.WriteLine("  dist/bsa-plan/bsa-pack-plan.json");
        writer.WriteLine("  dist/bsa-plan/bsa-entry-list.txt");
        writer.WriteLine("  dist/bsa-plan/loose-file-plan.json");
        writer.WriteLine("  dist/bsa-plan/bsa-validation.json");
        writer.WriteLine("  dist/bsa-plan/bsa-summary.md");
        writer.WriteLine("  dist/bsa-plan/build-manifest.json");
        writer.WriteLine("  dist/bsa-plan/checksums.sha256");
        writer.WriteLine();
        writer.WriteLine("Examples:");
        writer.WriteLine("  forge package fixtures/projects/ExampleMod --format json");
        writer.WriteLine("  forge package fixtures/projects/ExampleMod --target reports --format json");
        ...
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target mod-package --format json");
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target fomod --format json");
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target bsa-plan --bsa-plugin Synthetic.esp --format json");
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target bsa-plan --verify-existing --format json");
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target bsa-bsarch --packer C:\\Tools\\bsarch.exe --dry-run --format json");
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target bsa-bsarch --packer C:\\Tools\\bsarch.exe --approve <preview-sha256> --format json");
        writer.WriteLine("  forge package fixtures/projects/ExampleMod --target geck-handoff --format json");
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target mod-package --mo2-mods-root D:\\MO2\\mods --mo2-mod-name CombinedModExample --dry-run --format json");
        writer.WriteLine("  forge package fixtures/projects/CombinedModExample --target mod-package --reuse-existing-package --mo2-mods-root D:\\MO2\\mods --mo2-mod-name CombinedModExample --format json");
    ...
    private static void WriteReleaseVerifyHelp(TextWriter writer)
    {
        ...
        writer.WriteLine("  dist/release-dry-run/release-evidence-collection-plan.json");
    ...
    private static void WriteReleasePrepareHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Writes local staging/release-payload.json, release-archive-plan.json, archives/release.zip, release-archive-evidence.json, release-plan.json, release-summary.json, build-manifest.json, and checksums.sha256. The output root must stay under project dist/ and defaults to dist/release-prepare.");
        writer.WriteLine("--dry-run reports the same plan without writing files.");
        writer.WriteLine("When complete dist/fomod evidence exists, it verifies and stages package.zip plus FOMOD manifest, build-manifest, and checksums under staging/distributable and includes them in the deterministic local release ZIP. It does not rebuild or execute the installer, publish releases, call remote repositories, sign or attest artifacts, mutate plugins, automate MO2 or GECK, run runtime probes, or use AI.");
        writer.WriteLine("When dist/bsa-plan exists, release preparation requires its seven-file plan/report set to pass read-only verification and records exact source digests in release-payload.json and build-manifest.json. The plan is evidence only and is not inserted into the installable FOMOD archive.");
        writer.WriteLine();
        ...
        writer.WriteLine("  dist/release-prepare/release-archive-plan.json");
        writer.WriteLine("  dist/release-prepare/archives/release.zip");
        writer.WriteLine("  dist/release-prepare/release-archive-evidence.json");
