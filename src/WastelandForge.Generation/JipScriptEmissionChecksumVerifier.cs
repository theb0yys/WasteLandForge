...
public static class JipScriptEmissionChecksumVerifier
{
    public const string RuleId = "WF-GEN-008";
        ...
        LogicalId? projectId)
    {
        ...
        foreach (var expectedEntry in expectedEntries)
        {
            ...
            issues.Add(CreateIssue(
                ...
                checksumsPath,
                projectId,
                "JIP emission checksum entry is missing",
        ...
        foreach (var entry in checksumEntries.Values.OrderBy(entry => entry.Path, StringComparer.Ordinal))
        {
            ...
            issues.Add(CreateIssue(
                ...
                checksumsPath,
                projectId,
                "JIP emission checksum entry is not expected",
                $"checksums.sha256 records '{entry.Path}', but the JIP emission manifest does not declare that checksum entry."));
        }
        ...
        List<DiagnosticIssue> issues)
    {
        try
        {
            ...
            {
                issues.AddRange(JipScriptFileEmitter.ValidateManifestJson(projectRoot, manifestPath, manifest, projectId));
                return manifest;
            }
        }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException or JsonException)
        {
            issues.Add(CreateIssue(
                ...
                manifestPath,
                projectId,
                "JIP emission manifest evidence could not be read",
                $"Could not read JIP emission manifest evidence '{ToDisplayPath(projectRoot, manifestPath)}': {exception.Message}"));
            return null;
        }
        ...
        issues.Add(CreateIssue(
            ...
            manifestPath,
            projectId,
            "JIP emission manifest evidence is not an object",
            $"Expected JIP emission manifest evidence '{ToDisplayPath(projectRoot, manifestPath)}' to be a JSON object."));
        return null;
    }
        ...
        List<DiagnosticIssue> issues)
    {
        ...
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
            issues.Add(CreateIssue(
                ...
                checksumsPath,
                projectId,
                "JIP emission checksum evidence could not be read",
                $"Could not read JIP emission checksum evidence '{ToDisplayPath(projectRoot, checksumsPath)}': {exception.Message}"));
            return null;
        }
    }
        ...
        foreach (var output in outputs.OfType<JsonObject>())
        {
            ...
            if (!IsInsideOrEqual(outputRoot, fullPath))
            {
                issues.Add(CreateIssue(
                    ...
                    manifestPath,
                    projectId,
                    "JIP emission checksum expected path must stay under emission root",
                    $"Expected manifest output path '{outputPath}' to stay under generated JIP emission root '{ToDisplayPath(projectRoot, outputRoot)}'."));
                continue;
            }
        ...
        for (var index = 0; index < lines.Length; index++)
        {
                ...
                string.IsNullOrWhiteSpace(rawPath.Trim()))
            {
                issues.Add(CreateIssue(
                    ...
                    checksumsPath,
                    projectId,
                    "JIP emission checksum entry is malformed",
                    $"Expected checksums.sha256 line {index + 1} to contain a 64-character SHA-256 hex digest, two spaces, and an emission-root-relative file path."));
                continue;
            }
            ...
            if (!TryNormalizeChecksumEntryPath(outputRoot, parsedPath, out var normalizedPath))
            {
                issues.Add(CreateIssue(
                    ...
                    checksumsPath,
                    projectId,
                    "JIP emission checksum path must stay under emission root",
                    $"Expected checksums.sha256 line {index + 1} path '{parsedPath}' to stay under generated JIP emission root."));
                continue;
            }
            ...
            if (duplicatePath is not null)
            {
                issues.Add(CreateIssue(
                    ...
                    checksumsPath,
                    projectId,
                    "JIP emission checksum entry is duplicated",
                    $"Expected checksums.sha256 to record '{duplicatePath}' once, but line {index + 1} records duplicate entry '{normalizedPath}'."));
                continue;
            }

            entries[normalizedPath] = new ChecksumEntry(normalizedPath, sha256.ToLowerInvariant(), index + 1);
        }
        ...
        List<DiagnosticIssue> issues)
    {
        ...
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
            issues.Add(CreateIssue(
                ...
                filePath,
                projectId,
                "JIP emission checksum file entry could not be read",
        ...
        issues.Add(CreateIssue(
            ...
            filePath,
            projectId,
            "JIP emission checksum digest does not match file",
    ...
    private static DiagnosticIssue CreateIssue(
        ...
        new(
            ...
            projectId,
            suggestedFix: "Regenerate JIP emission evidence so the manifest, generated scripts, and checksum sidecar agree.",
    ...
    private sealed record ChecksumEntry(string Path, string Sha256, int LineNumber);
}
