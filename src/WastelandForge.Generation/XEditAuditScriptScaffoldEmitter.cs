...
public sealed class XEditAuditScriptScaffoldEmitter
{
    ...
    {
        if (StringComparer.Ordinal.Equals(audit.Intent, "check-for-errors")) return CreateCheckContent(audit);
    ...
    private static string CreateCheckContent(XEditAuditAdapterPlanEntry audit)
    {
        var builder = new StringBuilder();
        builder.AppendLine("{"); builder.AppendLine("  Generated read-only xEdit Check audit by WastelandForge."); builder.AppendLine($"  Audit: {audit.AuditId}"); builder.AppendLine($"  Expected report: {audit.ReportPath}"); builder.AppendLine("  Forge does not execute this script. Run it manually in xEdit against the declared subject plugin."); builder.AppendLine("}");
        builder.AppendLine("unit WastelandForgeCheckAudit;"); builder.AppendLine("var Findings: TStringList; RecordsVisited: Integer;"); builder.AppendLine();
        builder.AppendLine("function JsonEscape(Value: string): string;"); builder.AppendLine("begin"); builder.AppendLine("  Result := StringReplace(Value, '\\', '\\\\', [rfReplaceAll]);"); builder.AppendLine("  Result := StringReplace(Result, '" + '"' + "', '\\" + '"' + "', [rfReplaceAll]);"); builder.AppendLine("  Result := StringReplace(Result, #13#10, '\\n', [rfReplaceAll]);"); builder.AppendLine("end;"); builder.AppendLine();
        builder.AppendLine("function Initialize: integer;"); builder.AppendLine("begin"); builder.AppendLine("  Findings := TStringList.Create;"); builder.AppendLine("  RecordsVisited := 0;"); builder.AppendLine("  Result := 0;"); builder.AppendLine("end;"); builder.AppendLine();
        builder.AppendLine("function Process(e: IInterface): integer;"); builder.AppendLine("var MessageText: string;"); builder.AppendLine("begin"); builder.AppendLine("  Result := 0;");
        builder.Append("  if not ("); for (var i=0;i<audit.RecordTypes.Count;i++){if(i>0)builder.Append(" or ");builder.Append("(Signature(e) = '").Append(EscapePascalString(audit.RecordTypes[i])).Append("')");} builder.AppendLine(") then exit;");
        builder.AppendLine("  Inc(RecordsVisited);"); builder.AppendLine("  MessageText := Check(e);"); builder.AppendLine("  if MessageText = '' then exit;");
        builder.AppendLine("  Findings.Add('{\"message\":\"' + JsonEscape(MessageText) + '\",\"recordFile\":\"' + JsonEscape(GetFileName(GetFile(e))) + '\",\"signature\":\"' + Signature(e) + '\",\"fixedFormId\":\"' + IntToHex(FixedFormID(e), 8) + '\",\"editorId\":\"' + JsonEscape(EditorID(e)) + '\",\"loadOrderFormId\":\"' + IntToHex(GetLoadOrderFormID(e), 8) + '\"}');");
        builder.AppendLine("end;"); builder.AppendLine(); builder.AppendLine("function Finalize: integer;"); builder.AppendLine("begin"); builder.AppendLine("  { The manual handoff must wrap these findings with Forge script/plugin digest metadata before ingestion. }"); builder.AppendLine($"  Findings.SaveToFile(ProgramPath + '{EscapePascalString(Path.GetFileName(audit.ReportPath))}.findings.jsonl');"); builder.AppendLine("  Findings.Free;"); builder.AppendLine("  Result := 0;"); builder.AppendLine("end;");
        return builder.ToString().ReplaceLineEndings("\n");
    }
