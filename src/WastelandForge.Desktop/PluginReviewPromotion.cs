...
internal static class PluginReviewPromotion
{
    ...
    public static PluginReviewPreview Preview(string projectRoot, PluginReviewInput input)
    {
        try
        {
            ...
            if (File.Exists(reportDestination) || File.Exists(evidencePath)) throw new InvalidOperationException("Review report snapshot or evidence already exists; v0.1 promotion is create-only.");
