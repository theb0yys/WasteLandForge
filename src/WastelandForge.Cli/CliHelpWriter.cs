...
internal static class CliHelpWriter
{
    ...
    private static void WriteGenerateHelp(TextWriter writer)
    {
        ...
        writer.WriteLine("Target 'geck-authoring-plan' writes only deterministic plan evidence under generated/geck-authoring-plan. --dry-run performs no writes. It never launches GECK/xEdit or writes plugin bytes.");
        writer.WriteLine("Target 'geck-authoring-subject-handoff' writes a deterministic manual GECK worklist, subject contract, creation-notes template, manifest, and checksums bound to the current resolved plan. It never launches tools, writes plugin bytes, verifies a plugin, grants approval, or selects a license.");
