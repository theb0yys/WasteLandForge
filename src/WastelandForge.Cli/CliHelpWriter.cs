...
internal static class CliHelpWriter
{
    ...
    private static void WritePackageHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Usage:");
        writer.WriteLine("  forge package [project-root] [--project <path>] [--target reports|mcm-json|jip-scripts] [--output dist/<name>] [--summary <path>] [--format human|plain|json|sarif|github] [--dry-run] [--verify-existing] [--no-input]");
        writer.WriteLine();
        writer.WriteLine("Assembles deterministic package staging trees under project dist/. Target 'reports' writes local package-plan, staging-layout, package.zip, package-archive-evidence, build-manifest, checksum, present/missing input-discovery evidence, copied present report evidence, and archive digest/entry/timestamp evidence under dist/reports-package. Target 'mcm-json' writes MCM JSON, translations, staged referenced texture assets, schema-validated package-manifest.json, schema-validated install-preview.json, install-preview.md, schema-validated install-plan.json, install-plan.md, schema-validated package-verification.json, package-verification.md, reusable package-verification evidence cross-checks, entry-validated package.zip with internal archive entry revalidation, build-manifest.json, and checksums.sha256. Target 'jip-scripts' writes a loose-file package staging tree under dist/jip-scripts/package/Data/nvse/plugins/scripts, package-manifest.json, install-plan.json, build-manifest.json, and checksums.sha256.");
