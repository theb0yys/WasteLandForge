...
public sealed record PluginArtifactDefinition(string Id, string File, string FullPath, string PluginType, string DataPath, string Sha256, long Length, string AuthoringTool, string ReviewStatus, string? ReviewEvidence, string RegistryFile);
...
public static class PluginArtifactRegistryReader
{
    ...
    private static PluginArtifactDefinition ReadPlugin(string root, string registryFile, JsonObject item)
    {
        ...
        if (evidence is not null && !File.Exists(ResolveContained(root, evidence, "plugin review evidence"))) throw new InvalidOperationException("Plugin review evidence is missing.");
        return new(id, file.Replace('\\', '/'), full, type, dataPath, sha, length, tool, review, evidence, Path.GetRelativePath(root, registryFile).Replace('\\', '/'));
    }
