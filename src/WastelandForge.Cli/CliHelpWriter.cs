...
internal static class CliHelpWriter
{
    ...
    private static void WriteReleaseHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Release verify is implemented. Release prepare writes Gate 268 local staging-payload, release-archive-plan, deterministic release archive, release-archive-evidence, release-plan, release-summary, build-manifest, and checksum evidence only. Release publish runs Gate 280 no-publish governance preflight with local evidence shape classification, checksum sidecar digest revalidation, build-manifest output digest revalidation, release-archive-evidence metadata cross-reference, archive digest metadata revalidation, archive entry metadata revalidation, and semantic release-evidence validation, then refuses real publish by default.");
    }
