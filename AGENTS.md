...
- ADR-013: Forge may orchestrate approval-bound plugin record authoring through a capability-versioned external provider when the exact deterministic plan is locally resolved, digest-approved, and independently verified. Forge core must not write raw ESP/ESM bytes; provider journals and UI observations are not canonical truth; no execution is authorized before the verifier and provider-discovery gates pass.
