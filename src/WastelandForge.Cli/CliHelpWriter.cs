...
internal static class CliHelpWriter
{
    ...
    private static void WritePackageHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Usage:");
        writer.WriteLine("  forge package [project-root] [--project <path>] [--target mcm-json] [--output dist/<name>] [--format human|plain|json] [--dry-run] [--verify-existing] [--no-input]");
        writer.WriteLine();
        writer.WriteLine("Assembles the Gate 88 deterministic MCM Extender package staging tree under project dist/. Target 'mcm-json' writes MCM JSON, translations, staged referenced texture assets, schema-validated package-manifest.json, schema-validated install-preview.json, install-preview.md, schema-validated package-verification.json, package-verification.md, reusable package-verification evidence cross-checks, entry-validated package.zip with internal archive entry revalidation, build-manifest.json, and checksums.sha256. Use --verify-existing to read existing package evidence and run the file-based verifier without regenerating outputs.");
