...
internal static class CliHelpWriter
{
    ...
    private static void WriteGenerateHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Writes deterministic generated artifacts under project generated/. Target 'reports' writes metadata reports; target 'mcm-json' writes Gate 114 MCM Extender JSON, runtime requirements, translations, header and image options, keybind options, checkbox options, string-toggle options, staged referenced texture assets, a schema-validated loose-file package manifest, a schema-validated install-preview report, a human install-preview summary, a schema-validated package-verification report, a human package-verification summary, and reusable package-verification evidence cross-checks, then validates the output schema and MCM image asset references.");
    ...
    private static void WriteBuildHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Runs deterministic build targets and writes local build evidence under project dist/. Target 'reports' writes metadata reports; target 'mcm-json' writes Gate 114 MCM Extender JSON, runtime requirements, translations, header and image options, keybind options, checkbox options, string-toggle options, staged referenced texture assets, a schema-validated loose-file package manifest, a schema-validated install-preview report, a human install-preview summary, a schema-validated package-verification report, a human package-verification summary, reusable package-verification evidence cross-checks, and package.zip with entry validation plus internal archive entry revalidation, then validates the output schema and MCM image asset references.");
    ...
    private static void WritePackageHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Assembles the Gate 114 deterministic MCM Extender package staging tree under project dist/. Target 'mcm-json' writes MCM JSON, translations, staged referenced texture assets, schema-validated package-manifest.json, schema-validated install-preview.json, install-preview.md, schema-validated package-verification.json, package-verification.md, reusable package-verification evidence cross-checks, entry-validated package.zip with internal archive entry revalidation, build-manifest.json, and checksums.sha256. Use --verify-existing to read existing package evidence and run the file-based verifier, including checksum-file, checksum malformed-entry, checksum path containment, checksum unexpected-entry, checksum duplicate-entry, checksum case-insensitive duplicate-entry, checksum canonical-order, checksum digest canonical-casing, checksum path separator canonicalization, checksum path casing, checksum blank-line, checksum entry spacing, checksum line-ending, checksum trailing-newline, build-manifest content, install-preview summary content, install-preview/package-manifest entry cross-check, package-verification summary content, package-verification JSON check content, package-verification JSON metadata content revalidation, package-verification archive detail content revalidation, install-preview archive detail content revalidation, package-manifest archive detail content revalidation, archive detail cross-report consistency revalidation, and package archive presence revalidation, without regenerating outputs. SARIF, GitHub, and Markdown summary output are available only with --verify-existing.");
