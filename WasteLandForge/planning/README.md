...
- Gate 533 - implement the deterministic first-slice xEdit Pascal observer bundle, immutable raw-observation schema, Forge report sealer, CLI/help wiring, and synthetic refusal/acceptance coverage without launching xEdit or using real plugin bytes.
- Gate 534 - only after separate approval and an explicitly supplied redistributable synthetic plugin, run the manual FNVEdit compatibility smoke or record it deferred; no authoring execution follows automatically.
