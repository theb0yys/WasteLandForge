...
- `/forge generate --target mcm-json`, `/forge build --target mcm-json`, and
  ...
  available. The output is the Gate 78 validated MCM Extender JSON runtime
  ...
  `package-verification.json`, `package.zip`, `build-manifest.json`, and
  `checksums.sha256` under `dist/mcm-json`. The package manifest is validated
  against `package-manifest/0.1.0`, and
  ...
  package evidence, package counts, and archive validation status. These
  remain reports only: they list Data-relative would-copy paths and do not
  install into Data or MO2. It supports header,
