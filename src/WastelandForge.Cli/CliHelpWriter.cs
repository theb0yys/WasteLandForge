...
internal static class CliHelpWriter
{
    ...
    private static void WriteGenerateHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Writes deterministic generated artifacts under project generated/. Target 'reports' writes metadata reports; target 'mcm-json' writes Gate 80 MCM Extender JSON, runtime requirements, translations, header and image options, keybind options, checkbox options, string-toggle options, staged referenced texture assets, a schema-validated loose-file package manifest, a schema-validated install-preview report, a human install-preview summary, a schema-validated package-verification report, and a human package-verification summary, then validates the output schema and MCM image asset references.");
    ...
    private static void WriteBuildHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Runs deterministic build targets and writes local build evidence under project dist/. Target 'reports' writes metadata reports; target 'mcm-json' writes Gate 80 MCM Extender JSON, runtime requirements, translations, header and image options, keybind options, checkbox options, string-toggle options, staged referenced texture assets, a schema-validated loose-file package manifest, a schema-validated install-preview report, a human install-preview summary, a schema-validated package-verification report, a human package-verification summary, and package.zip with entry validation, then validates the output schema and MCM image asset references.");
    ...
    private static void WritePackageHelp(TextWriter writer)
    {
        ...
        writer.WriteLine();
        writer.WriteLine("Assembles the Gate 80 deterministic MCM Extender package staging tree under project dist/. Target 'mcm-json' writes MCM JSON, translations, staged referenced texture assets, schema-validated package-manifest.json, schema-validated install-preview.json, install-preview.md, schema-validated package-verification.json, package-verification.md, entry-validated package.zip, build-manifest.json, and checksums.sha256.");
